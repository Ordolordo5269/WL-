...
export function usePoliticsData(countryName: string | null, iso3: string | null, enabled: boolean = true): UsePoliticsDataReturn {
  ...
  const refetch = useCallback(() => {
    if (countryName && iso3 && enabled) fetchData(countryName, iso3);
  }, [countryName, iso3, fetchData, enabled]);

  useEffect(() => {
    if (countryName && iso3 && enabled) {
  ...
  }, [countryName, iso3, enabled, fetchData]);
