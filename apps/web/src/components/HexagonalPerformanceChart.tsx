...
export default function HexagonalPerformanceChart({
  ...
  useEffect(() => {
    ...
    const createPolygonPath = (data: Record<string, number>, _color: string, _opacity: number = 0.3) => {
    ...
    // Draw data points
    polygon1.points.slice(0, -1).forEach((point, _i) => {
      svg.append('circle')
        .attr('cx', point[0])
        .attr('cy', point[1])
        .attr('r', 5)
    ...
    polygon2.points.slice(0, -1).forEach((point, _i) => {
      svg.append('circle')
        .attr('cx', point[0])
        .attr('cy', point[1])
        .attr('r', 5)
    ...
    dimensions.forEach((dim, i) => {
      ...
      // labelRadius used for label positioning
