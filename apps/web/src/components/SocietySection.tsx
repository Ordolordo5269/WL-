...
export default function SocietySection({ data, isLoading, error, series: _series, fetchIndicatorSeries, iso3 }: SocietySectionProps) {
    ...
    >
      ...
      {false && iso3 && fetchIndicatorSeries && (
        <div className="mt-6 pt-6 border-t border-slate-700/50">
          ...
          {selectedIndicator ? (
            (() => {
              const currentIndicatorSlug = selectedIndicator as string;
              const indicator = SOCIETY_INDICATORS.find(ind => ind.slug === currentIndicatorSlug);
              if (!indicator) return null;

              const chartData = timeSeriesData[currentIndicatorSlug] || [];
              const isLoading = loadingChart[currentIndicatorSlug];
              const chartError = chartErrors[currentIndicatorSlug];
                ...
                >
                  <div className="historical-chart-header">
                    <h4 className="historical-chart-title">
                      {getIndicatorDisplayName(currentIndicatorSlug)}
                    </h4>
                    {chartData.length > 0 && (
                  ...
                  {chartError ? (
                    ...
                    <div className="historical-chart-container">
                      <TimeSeriesChart
                        data={chartData}
                        indicatorName={getIndicatorDisplayName(currentIndicatorSlug)}
                        height={300}
                        color={indicator?.color}
                        isLoading={isLoading}
                      />
