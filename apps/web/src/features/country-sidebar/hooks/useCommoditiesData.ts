...
export interface UseCommoditiesDataReturn {
...
export function useCommoditiesData(iso3: string | null, countryName: string | null, enabled: boolean = true): UseCommoditiesDataReturn {
  const [data, setData] = useState<TCommoditiesData | null>(null);
  ...
  const fetchData = useCallback(async (code3: string, name: string | null) => {
    ...
    setError(null);
    try {
      const result = await commoditiesService.getCommoditiesData(code3, name);
      if (currentRequestId !== requestIdRef.current) return;
      setData(result);
    } catch (err) {
      if (currentRequestId !== requestIdRef.current) return;
      const message = err instanceof Error ? err.message : 'Failed to load raw materials data';
      setError(message);
      setData(null);
    } finally {
