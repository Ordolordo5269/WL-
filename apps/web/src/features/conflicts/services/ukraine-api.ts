...
export interface MapboxMap {
  ...
  on: (event: string, layerOrHandler: string | ((e: any) => void), handler?: (e: any) => void) => void;
...
export class UkraineAPIService {
   ...
   */
  static addUkraineLayers(map: MapboxMap, data: UkraineFrontlineData): void {
    try {
   ...
   */
  static removeUkraineLayers(map: MapboxMap): void {
    const layerIds = [
      'ukraine-polygons',
      'ukraine-outlines', 
      'ukraine-lines',
   ...
   */
  static startAutoRefresh(map: MapboxMap): void {
   ...
   */
  static async loadUkraineDataWithFeedback(map: MapboxMap): Promise<void> {
    try {
