// NewsAPI proxy service - keeps the API key server-side
// Proxies requests to NewsAPI.org so the frontend never exposes the key
...
function getCached(key: string): NewsAPIArticle[] | null {
...
function setCache(key: string, data: NewsAPIArticle[]): void {
...
export async function fetchConflictNews(pageSize: number = 20): Promise<NewsAPIArticle[]> {
  ...
  const url = `https://newsapi.org/v2/everything?q=${encodeURIComponent(query)}&sortBy=publishedAt&pageSize=${pageSize}&language=en&apiKey=${getApiKey()}`;
...
export async function fetchTopConflictHeadlines(pageSize: number = 15): Promise<NewsAPIArticle[]> {
  ...
  const url = `https://newsapi.org/v2/everything?q=${encodeURIComponent(query)}&sortBy=popularity&pageSize=${pageSize}&language=en&apiKey=${getApiKey()}`;
